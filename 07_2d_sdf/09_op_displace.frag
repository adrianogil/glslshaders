...
vec2 opRepLim(vec2 p, float c, vec2 l)
{
    return p - c * clamp(floor( (p/c) + 0.5), -l, l);
}

float opDisplace(vec2 uv, float scaleX, float scaleY)
{
    return sin(scaleX * uv.x + scaleY * uv.y);
}
...
vec3 drawScene(vec2 uv) {
    ...
    uv = opRepLim(uv, 0.15, vec2(1,2));

    float circle = sdfCircle(uv, 0.05, vec2(0, 0));

    float square = sdfSquare(uv, 0.05, vec2(0.05, 0.), 0.);

    float res;
    res = min(circle, square);
    res = res + opDisplace(uv, 0.3, 0.1);
